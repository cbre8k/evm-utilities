...
export const OPCODES: OpcodeInfo[] = [
  ...
  ...Array.from({ length: 32 }, (_, i) => ({
    ...
    name: `PUSH${i + 1}`,
    gas: 3,
    category: 'stack' as OpcodeCategory,
...
export const OPCODE_BY_NAME = new Map<string, OpcodeInfo>(
  OPCODES.map(o => [o.name, o]),
);

export const OPCODE_BY_HEX = new Map<string, OpcodeInfo>(
  OPCODES.map(o => [o.hex, o]),
);

export function getOpcodeInfo(nameOrHex: string): OpcodeInfo | undefined {
  return OPCODE_BY_NAME.get(nameOrHex.toUpperCase()) ?? OPCODE_BY_HEX.get(nameOrHex.toLowerCase());
}
