// ============================================================
// shared/constants/networks.ts — Network configuration
// ============================================================

export type Network = {
  id: string;
  name: string;
  fullnodeRpcUrls: string[];
  archiveRpcUrls: string[];
};

export const NETWORKS: Network[] = [
  {
    id: 'mainnet',
    name: 'MAINNET',
    fullnodeRpcUrls: ['https://eth.blockrazor.xyz', 'https://mainnet.rpc.sentio.xyz', 'https://ethereum-rpc.publicnode.com', 'https://api.zan.top/eth-mainnet', 'https://ethereum.public.blockpi.network/v1/rpc/public'],
    archiveRpcUrls: ['https://mainnet.chainnodes.org/9b5faff0-f503-47d3-982e-5fcacadd3e9a'],
  },
  {
    id: 'bsc',
    name: 'BSC',
    fullnodeRpcUrls: ['https://binance-smart-chain-public.nodies.app', 'https://bsc.meowrpc.com', 'https://bsc.blockrazor.xyz'],
    archiveRpcUrls: ['https://bsc-mainnet.chainnodes.org/9b5faff0-f503-47d3-982e-5fcacadd3e9a'],
  },
  {
    id: 'arbitrum',
    name: 'ARBITRUM',
    fullnodeRpcUrls: ['https://arbitrum.meowrpc.com', 'https://rpc.ankr.com/arbitrum', 'https://api.zan.top/arb-one', 'https://arbitrum.public.blockpi.network/v1/rpc/public'],
    archiveRpcUrls: ['https://arbitrum-one.chainnodes.org/9b5faff0-f503-47d3-982e-5fcacadd3e9a'],
  },
  {
    id: 'optimism',
    name: 'OPTIMISM',
    fullnodeRpcUrls: ['https://mainnet.optimism.io', 'https://rpc.ankr.com/optimism', 'https://api.zan.top/opt-mainnet', 'https://optimism-public.nodies.app'],
    archiveRpcUrls: ['https://optimism-mainnet.chainnodes.org/9b5faff0-f503-47d3-982e-5fcacadd3e9a'],
  },
  {
    id: 'base',
    name: 'BASE',
    fullnodeRpcUrls: ['https://mainnet.base.org', 'https://api.zan.top/base-mainnet', 'https://developer-access-mainnet.base.org', 'https://base.drpc.org'],
    archiveRpcUrls: ['https://base-mainnet.chainnodes.org/9b5faff0-f503-47d3-982e-5fcacadd3e9a'],
  },
  {
    id: 'viction',
    name: 'VICTION',
    fullnodeRpcUrls: ['https://viction.blockpi.network/v1/rpc/public', 'https://viction.drpc.org', 'https://rpc.viction.xyz'],
    archiveRpcUrls: ['https://vic-mainnetd-rpc.tforce.dev/'],
  },
  {
    id: 'custom',
    name: 'CUSTOM',
    fullnodeRpcUrls: [''],
    archiveRpcUrls: [''],
  },
];
