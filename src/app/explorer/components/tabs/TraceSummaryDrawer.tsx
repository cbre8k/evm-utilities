...
function serializeTrace(items: TraceItem[], indent = 0): string {
  ...
  for (const item of items) {
    if (item.kind === 'frame') {
      ...
      const from = e.node.from?.slice(0, 10) ?? '?';
      const to = e.node.to?.slice(0, 10) ?? '?';
      ...
      lines.push(`${pad}JUMP ${item.address?.slice(0, 10) ?? '?'} (gas: ${item.gasUsed})`);
      lines.push(...serializeTrace(item.items, indent + 1).split('\n').filter(Boolean));
    } else {
      const e = item.entry;
      if (e.kind === 'event') {
        lines.push(`${pad}LOG ${e.name ?? e.opcode} @ ${e.address.slice(0, 10)}`);
      } else if (e.kind === 'storage' && e.opcode === 'SSTORE') {
        lines.push(`${pad}SSTORE slot=${e.slot.slice(0, 10)}…`);
      }
    }
  }
  return lines.join('\n');
