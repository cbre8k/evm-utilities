...
export function layoutCallGraphNodes(graphNodes: GraphNode[]) {
  const levels = new Map<string, number>();
  const byId = new Map(graphNodes.map((n) => [n.id, n] as const));
  const roots = graphNodes.filter((node) => {
    return !graphNodes.some((other) => other.children.includes(node.id));
  });

  const queue: Array<{ id: string; depth: number }> = roots.map((node) => ({ id: node.id, depth: 0 }));
  while (queue.length > 0) {
    const { id, depth } = queue.shift()!;
    if (levels.has(id) && levels.get(id)! <= depth) continue;
    levels.set(id, depth);
    const node = byId.get(id);
    if (!node) continue;
    node.children.forEach((childId) => queue.push({ id: childId, depth: depth + 1 }));
  }
  ...
  [...grouped.entries()].sort((a, b) => a[0] - b[0]).forEach(([level, nodes]) => {
