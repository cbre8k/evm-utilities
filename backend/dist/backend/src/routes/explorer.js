...
function asHexQuantity(value, fallback = '0x0') {
...
function normalizeTenderlyCall(node, parentId, index = 0) {
    ...
    return {
        ...
        logs: node?.logs ?? [],
        children: children.map((child, childIndex) => normalizeTenderlyCall(child, id, childIndex)),
    };
}
function buildTenderlyOverview(trace) {
